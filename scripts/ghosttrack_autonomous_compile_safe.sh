...
echo "📁 [2] Inizializzazione config/modules.yaml"
echo "" > config/modules.yaml

echo "🧩 [3] Aggregazione moduli da /api/status"
curl -s http://127.0.0.1:9090/api/status > tmp/api_status.json
MODS=$(jq -r '.modules | keys[]' tmp/api_status.json)
for mod in $MODS; do
  label=$(jq -r ".modules[\"$mod\"].description" tmp/api_status.json | sed 's/"/\\"/g')
...
echo "📚 [4] Integrazione voci da sidebar (screen)"
SIDEBAR_MODULES=(
...
echo "📊 [5] Rigenerazione dashboard"
DASH=dashboard.html
cat > "$DASH" <<'HTML'
...
if [ -d .git ]; then
  ...
  git commit -m "build: compilazione autonoma completa con moduli API, voci sidebar, dashboard e deploy"
  git push origin main || true
fi

echo "✅ GhostTrack-v2 ora è completo, coerente e pubblicato"
